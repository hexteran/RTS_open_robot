...
function logic(day, month,prc) 
	sber = -get_sber_direction(day,month) --  ƒЋя Si нужен минус, а дл€ RI нет
	LOGGING("Got SBER direction",tostring(sber))

	lkoh = -get_lkoh_direction(day,month) -- ƒЋя Si нужен минус, а дл€ RI нет
	LOGGING("Got LKOH direction",tostring(lkoh))

	gazp = -get_gazp_direction(day,month) -- ƒЋя Si нужен минус, а дл€ RI нет
